...
def windows_based_loss_diffusion(gt_fig, out_fig, frames_fig, n_frames, loss_fn=nn.MSELoss(reduction="none")):
    assert len(gt_fig.shape) == 4
    w, dim, timesteps, joints = gt_fig[:,:,12:,:].shape

    gt_fig = gt_fig[:,:,12:,:].transpose(0, 2, 3, 1).reshape(-1, timesteps*joints*dim)
    out_fig = out_fig[:,:,12:,:].transpose(0, 2, 3, 1).reshape(-1, timesteps*joints*dim)
    #torch.mean(loss_fn(gt_fig,out_fig).reshape(gt_fig.shape[0],-1),dim=-1).shape
...
def filter_vecs_by_cond(cond, out, gt_data, meta, frames):
    out = filter_by_cond(out, cond)
    gt_data = filter_by_cond(gt_data, cond)
    meta = filter_by_cond(meta, cond)
    frames = filter_by_cond(frames, cond)
    
    return out, gt_data, meta, frames
...
def padding(fig_reconstruction_loss, gt, pad_size):
